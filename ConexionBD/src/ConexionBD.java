...
public class ConexionBD extends JFrame {
    ...
    private void abrirRegistro() {
        ...
        JComboBox<String> rolComboBox = new JComboBox<>(new String[]{"admin", "empleado"}); // Solo permite admin y empleado
    ...
    public static void insertUser(Connection con, String name, String pass, String role) {
        String sql = "INSERT INTO users(name, pass, role) VALUES(?, ?, ?)";
        try (PreparedStatement pstmt = con.prepareStatement(sql)) {
            pstmt.setString(1, name);
            pstmt.setString(2, pass);
            pstmt.setString(3, role); // Asegúrate de insertar el rol
            pstmt.executeUpdate();
            System.out.println("Usuario insertado: " + name + " con rol: " + role);
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            throw new RuntimeException(e);
        }
